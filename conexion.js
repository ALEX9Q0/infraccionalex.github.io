...
const db = mysql.createConnection({
  host: 'localhost',
  user: 'root',
  password: 'alexnokia24', // Cambia esto según tu configuración
...
app.post('/api/users/create', async (req, res) => {
  ...
  try {
    ...
    const query = 'INSERT INTO ciudadanos (nombre, email, password) VALUES (?, ?, ?)';
  ...
  db.query(query, [numero_agente], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      if (password === agente.password) {
        ...
        return res.status(401).json({ message: 'Número de agente o contraseña incorrectos' });
      }
    } else {
      return res.status(401).json({ message: 'Número de agente o contraseña incorrectos' });
    }
...
app.post('/api/vehiculos/registrar', (req, res) => {
  ...
  const query = 'INSERT INTO vehiculos (placas, estado, marca, color, ciudadano_id) VALUES (?, ?, ?, ?, ?)';
  
  db.query(query, [placas, estado, marca, color, ciudadano_id], (err, resultado) => {
    if (err) {
      console.error('Error al registrar vehículo:', err);
      return res.status(500).json({ mensaje: 'Error al registrar el vehículo' });
    }
    res.status(201).json({ mensaje: 'Vehículo registrado con éxito', id: resultado.insertId });
  });
});
// -----------------------------------------------------------
...
app.get('/api/infracciones/agente/:agenteId', (req, res) => {
  ...
  db.query(query, [agenteId], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Error al obtener las infracciones' });
    }

    if (results.length > 0) {
      return res.status(200).json(results);
    } else {
      return res.status(404).json({ message: 'No se encontraron infracciones para este agente' });
    }
  });
});
...
app.delete('/api/infracciones/eliminar/:id', (req, res) => {
  ...
  db.query(query, [infraccionId], (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ mensaje: 'Error al eliminar la infracción', error: err.message });
    }
...
app.post('/api/infracciones/registrar', (req, res) => {
  const { agente_id, ciudadano_id, vehiculo_id, lugar, fecha, hora, descripcion, placas, color, estado, marca } = req.body;
  ...
  if (!agente_id || !lugar || !fecha || !hora || !descripcion || !placas || !color || !estado || !marca) {
  ...
  const insertInfraccionQuery = `
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  const values = [agente_id, ciudadano_id, vehiculo_id, lugar, fecha, hora, descripcion, placas, color, estado, marca];

  db.query(insertInfraccionQuery, values, (err, resultado) => {
    if (err) {
      console.error('Error al registrar infracción en la base de datos:', err);
      return res.status(500).json({ mensaje: 'Error al registrar la infracción', error: err.message });
    }

    console.log('Infracción registrada con éxito. ID:', resultado.insertId);
    res.status(201).json({ mensaje: 'Infracción registrada con éxito', id: resultado.insertId });
  });
});
...
app.put('/api/reportes/:id/estado', (req, res) => {
  ...
  db.query(query, [estado, id], (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ mensaje: 'Error al actualizar el estado del reporte.' });
    }
    ...
    res.status(200).json({ mensaje: 'Estado del reporte actualizado correctamente.' });
  });
});
...
app.get('/api/reportes/:id', async (req, res) => {
  ...
  try {
    const [rows] = await db.promise().query('SELECT * FROM reportes WHERE id = ?', [reporteId]);
    if (rows.length === 0) {
      return res.status(404).json({ mensaje: 'Reporte no encontrado' });
    }
...
app.post('/api/reportes', async (req, res) => {
  const { descripcion, numero_exterior, numero_interior, calle, codigo_postal, estado } = req.body;

  // Validación de los campos requeridos
  if (!descripcion || !numero_exterior || !calle || !codigo_postal) {
    return res.status(400).json({ mensaje: 'Los campos descripción, número exterior, calle y código postal son obligatorios' });
  }
  ...
  try {
    const query = `
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    const [result] = await db.promise().query(query, [descripcion, numero_exterior, numero_interior, calle, codigo_postal, estado || 'Pendiente']);
...
app.put('/api/reportes/:id', async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE id = ?
    `;
    const [result] = await db.promise().query(query, [descripcion, numero_exterior, numero_interior, calle, codigo_postal, estado, reporteId]);
