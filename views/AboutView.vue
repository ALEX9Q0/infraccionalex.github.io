      ...
      <div v-if="activeSection === 'reportes'" class="fade-in">
        ...
        <div v-for="reporte in reportes" :key="reporte.id" class="reporte-card">
          ...
          <div class="reporte-actions">
            <button 
              v-if="reporte.estado !== 'Atendido'" 
              @click="cambiarEstado(reporte.id, 'Atendido')" 
              class="btn-verde">
              Marcar como Atendido
            </button>
            <button 
              v-if="reporte.estado !== 'Pendiente'" 
              @click="cambiarEstado(reporte.id, 'Pendiente')" 
              class="btn-rojo">
...
async fetchInfracciones() {
  try {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3000/api/reportes/${reporteId}/estado`, { estado: nuevoEstado });
...
.reporte-card {
  ...
  border-radius: 8px;
  padding: 20px 40px; /* Más espacio horizontal */
  margin: 20px auto;
  max-width: 800px; /* Aumentar el ancho de las tarjetas */
...
.reporte-details p {
  margin: 8px 0; /* Espaciado uniforme entre líneas */
...
.reporte-actions {
  ...
  gap: 20px; /* Espacio más amplio entre botones */
  margin-top: 20px;
}
